...
const { generateToken, verifyToken, blacklistToken, refreshToken } = require('../../utils/jwtSecret');
const { hashPassword, comparePassword } = require('../helpers/hashPassword');
const { appLogger } = require('../../config/setting/logger.config');

module.exports = {
  register: async (user_name, role, user_email, user_password, user_contact) => {
    try {
      ...
      appLogger.info('User registered', { user_name, role, userId: user.user_id });
      return user;
    } catch (error) {
  ...
  login: async (user_email, user_password) => {
    try {
      ...
      appLogger.info(`User ${user.user_name} logged in`, { userId: user.user_id });
  ...
  forgotPassword: async (user_email) => {
    try {
      ...
      const resetToken = generateToken({ userId: user.user_id }, process.env.JWT_SECRET, { expiresIn: '1h' });
      appLogger.info(`Password reset token generated for user ${user.user_name}`, { resetToken });
      return resetToken;
    } catch (error) {
