...
const getUploadPath = (file) => {
  ...
  if (fileCategories.documents.includes(file.mimetype))
    return path.join(baseDir, 'Documents');
  if (fileCategories.csvFiles.includes(file.mimetype))
    return path.join(baseDir, 'CSV');
  if (fileCategories.videos.includes(file.mimetype))
    return path.join(baseDir, 'Videos');
  return path.join(baseDir, 'Others');
};

const generateFileUrl = (filename, directory) =>
